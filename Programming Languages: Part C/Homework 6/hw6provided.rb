...
class Board
  ...
  def move_left
    @current_block.move(-1, 0, 0) if !game_over? and @game.is_running?
    draw
  end
  ...
  def rotate_clockwise
    @current_block.move(0, 0, 1) if !game_over? and @game.is_running?
    draw
  end
...
class Tetris
  ...
  def draw_piece(piece, old = nil)
    old.each { |block| block.remove } if !old.nil? and piece.moved
